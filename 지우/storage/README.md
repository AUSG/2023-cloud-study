...
- **고객 제공 키 (SSE-C)** : 키는 고객이 관리하고, 암호화는 AWS에서 수행
- **S3 관리형 키 (SSE-S3**) : AWS 관리형 키로 암호화, 키 추적 및 감사 수행 X
- **KMS 관리형 키 (SSE-KMS)** : AWS 관리형 키로 암호화, 키 추적 및 감사 수행 O (추가 비용)
...
1. 고객 관리형 키
   ...
   - 고객이 키 관리 가능
   - (optional) 자동교체 주기 1년
   - 매달 프리티어 초과 사용량에 따라 요금 부과, AWS KMS 할당량에 대해 요금 계산

2. AWS 관리형 키
   - 고객이 키 관리 불가능
   - (required) 자동교체 주기 1년, 변경 불가능
   - 매달 부과되는 요금이 따로 없음
     - 프리티어 초과 사용량에 따라 요금 부과되지만, AWS가 일부 요금을 대신 부담

<br>

# ⍥ Datasync

- AWS 완전 관리형 서비스로, 데이터 마이그레이션 및 보호, 콜드 데이터 보관 등의 기능이 있음
- **온프레미스와 클라우드(AWS) 간**의 데이터 전송
- **AWS 스토리지 서비스 간**의 데이터 전송 (S3, EFS, FSx)
- 데이터 **암호화** 및 데이터 **무결성** 검증
- NFS, SMB 프토토콜, S3 API, HDFS 지원
